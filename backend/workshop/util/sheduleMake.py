from workshop.models import product_info, parameter_info, measure_plan_info, control_plan_info, workshop_info
from workshop.models import ValueTypeChoices, GraphTypeChoices
from workshop.util.utils import UNK, product_uid, par_uid, measure_plan_uid, control_plan_uid, DEBUG
...
def addWorkshop(name=UNK,productId=None,batchSize=5,windowSize=25,description=None,worker=None, **kwargs):
    ...
    try:
        ...
        workshop = workshop_info(measure_plan.uid,name,description)
        if worker:
...
def alterWorkshop(plan_id,**kwargs):
    ...
    if DEBUG:
        ...
        print('worker: ', [worker.name for worker in workshop.workers.all()])
